...
def parse_args():
    ...
    parser = argparse.ArgumentParser(
        description="Add wpscan result as a note to Wordpress events that triggered an alarm. It currently only uses the destination IP as the url."
    )
    parser.add_argument(
        "--time_range",
        "-t",
...
def add_wpscan_note(alarm):
    if len(alarm["events"]) > 0:
        ...
        try:
            cmd = [
                WPSCAN,  # Arguments for Version 3.7.4
                "--no-banner",
                "--random-user-agent",
                "--format",
                "cli-no-colour",
