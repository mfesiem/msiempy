...
def alarm_examples():
    ...
    print(alarms)
    print(alarms.get_text(fields=["triggeredDate", "acknowledgedDate", "alarmName"]))
    ...
    alarms_events_use_query = AlarmManager(
        ...
        event_filters=[
            ("Rule.msg", "Postfix")
        ],  # Please replace "Postfix" by a test rule name (aka event name)
        page_size=5,
    )
    alarms_events_use_query.load_data(
        use_query=True, extra_fields=Event.REGULAR_EVENT_FIELDS
    )
    ...
    print(
        alarms_events_use_query.get_text(
            fields=[
                "triggeredDate",
                "acknowledgedDate",
                "alarmName",
            ...
            get_text_nest_attr=dict(fields=["Rule.msg", "Alert.SrcIP", "Alert.DstIP"]),
        )
    )  # Events fields
    ...
    print(
        alarms.get_text(
            fields=["triggeredDate", "alarmName", "events"],
            max_column_width=80,
            get_text_nest_attr=dict(
                fields=[
                    "ruleName",
                    "srcIp",
                    "destIp",
...
def devtree_exaples():
    ...
    for ds in devtree:
        assert (
            ds["name"] != "msiempy_test_datasource_delete_me"
