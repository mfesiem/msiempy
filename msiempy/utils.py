...
def tfgettimes(timeFrame):
    ...
    if t == "LAST_MINUTE":
        ...
        times = (
            ...
            now.replace(hour=24, minute=59, second=59),
        )
        ...
        times = (
            ...
            yesterday.replace(hour=24, minute=59, second=59),
        )
...
def divide_times(first, last, time=0, slots=0, delta=0, min_slots=2):
    """
        Divide the time range based on another time, a delta or
        on a number of slots

        Return list of tuple
    """

    # parse the dates
    t1 = dateutil.parser.parse(first) if not isinstance(
        first, datetime) else first
    t2 = dateutil.parser.parse(last) if not isinstance(
        last, datetime) else last
        ...
        if time == 0:
            if delta == 0:
                raise AttributeError(
                    "Either time, slots or delta must be specified")
