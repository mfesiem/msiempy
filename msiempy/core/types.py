...
class NitroList(collections.UserList, NitroObject):
    ...
    def get_text(
        ...
        :Parameters:
            - `format` (`str`): Can be: 
              * ``"prettytable"``: Returns a table generated by prettytable use ``MSWORD_FRIENDLY`` format. OR. 
              * ``"csv"``: Returns data with header and comma separated values.
            - `fields` (`list[str]`): list of fields you want in the table. If `None` : default fields are returned by .keys attribute and sorted.
